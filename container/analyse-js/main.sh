...
function execute-getjs {
    mkdir /js
    getJS --insecure --nocolors --url $URL > js/js-urls.txt
}
...
function execute-secretfinder {
    ...
    for url in $(cat js/js-urls.txt)
    do
        python3 secretfinder/SecretFinder.py -i $url -o cli -g 'jquery;bootstrap;api.google.com'  >> js/secretfinder-results.txt
    done
